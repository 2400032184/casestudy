...
export default function AppointmentCard({ appointment, onCancel }) {
  ...
  return (
    <>
      <Card
        sx={{
          backgroundColor: "#ffffffff", // pastel container
          transition: "transform 0.2s",
          "&:hover": { transform: "scale(1.02)" },
        }}
      >
        <CardHeader
          title={<Typography variant="h6">{appointment.doctor}</Typography>}
        ...
        <CardContent>
          ...
          <Typography variant="body2">
            {appointment.date && appointment.time
              ? `${dayjs(appointment.date).format("DD/MM/YYYY")} ${dayjs(appointment.time).format("HH:mm")}`
              : ""}
          </Typography>
